...
export async function extractPostCommandLearnings(
  ...
  try {
    ...
    if (pageAnalysis) {
      const currentUrl = ((stagehand.context as any).activePage?.() ?? stagehand.context.pages()[0])?.url() ?? "";
...
export async function runLearn(
  ...
  sink?.info("Phase 1: Analyzing current page...");
  ...
  sink?.info("Phase 2: Discovering site structure...");
...
QA-relevant observations: loading times, animations, dynamic content, tooltips, potential edge cases`;
  ...
  sink?.info("Phase 3: Extracting structured knowledge...");
...
5. Any tips for interacting with this site efficiently`,
  ...
  if (pageUrls.length > 0 && !isFocused) {
    ...
    const browserPage = (stagehand.context as any).activePage?.() ?? stagehand.context.pages()[0];
    for (let i = 0; i < pageUrls.length; i++) {
      ...
      try {
        ...
        sink?.warn(`  Skipped ${pagePath} (navigation failed)`);
      }
      await memory.saveSiteKnowledge(knowledge);
    }
  }
